...
function App() {
  const [inputs, setInputs] = useState([
    { id: 1, value: 0, fromUnit: "second", toUnit: "minute", digits: "Select digits", result: 0 },
  ]);
  ...
  const handleAddInput = () => {
    if (inputs.length < 7) {
      ...
      setInputs([
        ...inputs,
        { id: newId, value: 0, fromUnit: "second", toUnit: "minute", digits: "Select digits", result: 0 },
      ]);
    }
  };
  ...
  return (
    <div className="App bg-slate-200">
      <h1 className="font-bold text-3xl pt-4">Time Unit Converter</h1>
      {inputs.map((input) => (
        <div key={input.id}>
          <input
            type="number"
